...
ANÁLISE CRÍTICA DO V4:
❌ Long accuracy: 37-41% (PÉSSIMO!)
❌ Overfitting: CV 73% → Test 69% (diferença 4%)
❌ Meta-learner: +0.24% (quase inútil)
❌ Removeu LSTM/CNN (perdeu diversidade)
❌ Class imbalance não resolvido

V5 SOLUÇÃO DEFINITIVA:
✅ 6 modelos (LGB, XGB, CatBoost, RF, LSTM, CNN) - DIVERSIDADE MÁXIMA
✅ Features avançadas de qualidade (60+)
✅ Focal Loss para penalizar erros em longs
✅ Anti-overfitting (early stopping, dropout, regularização)
✅ Optuna eficiente (20 trials com pruning)
✅ Weighted ensemble baseado em long/short performance
✅ Features de order flow e microstructure
✅ Threshold ajustado por classe
✅ Multi-period com validação robusta

RESULTADO ESPERADO: 54-58% META com Long/Short BALANCEADOS (50-55% cada)
"""
...
print("=" * 80)
print("🚀 ULTIMATE V5 - SCALPER EXTRAORDINÁRIO")
print("=" * 80)
print()
...
class ModelWrapper:
    ...
    def __init__(self, models_dict, scaler, feature_columns, has_dl=False,
        ...
        self.version = "V5_SCALPER_EXTRAORDINARIO"
    ...
    def predict(self, X):
        ...
        # Use different thresholds for long/short
        predictions = np.zeros(len(proba))
        predictions[proba[:, 1] >= self.long_threshold] = 1

        return predictions.astype(int)

    def _prepare_sequences(self, X, lookback=20):
...
def tune_lightgbm(X_train, y_train, n_trials=20):
    """Tuning LightGBM com Optuna (anti-overfitting)."""
...
def tune_xgboost(X_train, y_train, n_trials=20):
    """Tuning XGBoost com Optuna (anti-overfitting)."""
...
def create_lstm_model(input_shape, dropout=0.3):
...
def create_cnn_model(input_shape, dropout=0.3):
...
def train_ultimate_v5(df, days):
    """Treina modelo V5 EXTRAORDINÁRIO."""

    print()
    print(f"🎯 TREINANDO V5 EXTRAORDINÁRIO ({days} dias)...")
    print("=" * 80)
    ...
    # Class weights (penalizar MUITO erros em longs)
    n_shorts = (y_train == 0).sum()
    n_longs = (y_train == 1).sum()
    class_weight = {
        0: 1.0,
        1: (n_shorts / n_longs) * 2.0  # DOBRO para forçar aprender longs!
    }

    print(f"\n⚖️  Class weights: Short={class_weight[0]:.2f}, Long={class_weight[1]:.2f}")

    # SMOTE
    print("\n🔄 Aplicando SMOTE...")
    smote = SMOTE(random_state=42)
    X_train_balanced, y_train_balanced = smote.fit_resample(X_train_scaled, y_train)
    print(f"   Original: {len(X_train_scaled)} → Balanced: {len(X_train_balanced)}")
    ...
    print("\n" + "=" * 80)
    print("🚀 TREINANDO 6 MODELOS (ML + DL) - DIVERSIDADE MÁXIMA")
    print("=" * 80)
    ...
    lgb_model = lgb.LGBMClassifier(**lgb_params, class_weight=class_weight, random_state=42, verbose=-1)
    ...
    xgb_params['scale_pos_weight'] = class_weight[1]
    ...
    if use_dl:
        ...
        # Prepare sequences
        X_train_seq = []
        for i in range(lookback, len(X_train_balanced)):
            X_train_seq.append(X_train_balanced[i-lookback:i])
        X_train_seq = np.array(X_train_seq)
        y_train_seq = y_train_balanced[lookback:]
        ...
        # Train LSTM
        lstm_model = create_lstm_model(input_shape=(lookback, X_train_balanced.shape[1]), dropout=0.4)
        ...
        lstm_pred_train_full = np.zeros(len(X_train_balanced))
        lstm_pred_train_full[lookback:] = lstm_model.predict(X_train_seq, verbose=0).flatten()
        lstm_pred_train_full[:lookback] = lstm_pred_train_full[lookback]  # Fill início

        lstm_pred_test_full = np.zeros(len(X_test_scaled))
        lstm_pred_test_full[lookback:] = lstm_model.predict(X_test_seq, verbose=0).flatten()
        lstm_pred_test_full[:lookback] = lstm_pred_test_full[lookback]

        lstm_acc = accuracy_score(y_test_seq, (lstm_pred_test_full[lookback:] >= 0.5).astype(int))
        lstm_long_acc = accuracy_score(y_test_seq[y_test_seq==1],
                                       (lstm_pred_test_full[lookback:][y_test_seq==1] >= 0.5).astype(int))
        lstm_short_acc = accuracy_score(y_test_seq[y_test_seq==0],
                                        (lstm_pred_test_full[lookback:][y_test_seq==0] >= 0.5).astype(int))

        models['lstm'] = lstm_model
        base_predictions_train.append(lstm_pred_train_full)
        base_predictions_test.append(lstm_pred_test_full)
        base_accuracies.append(lstm_acc)

        print(f"   ✅ LSTM: {lstm_acc*100:.2f}% (Long: {lstm_long_acc*100:.2f}%, Short: {lstm_short_acc*100:.2f}%)")

        # 6. CNN
        print("\n6/6 - CNN...")

        cnn_model = create_cnn_model(input_shape=(lookback, X_train_balanced.shape[1]), dropout=0.4)
        ...
        cnn_pred_train_full = np.zeros(len(X_train_balanced))
        cnn_pred_train_full[lookback:] = cnn_model.predict(X_train_seq, verbose=0).flatten()
        cnn_pred_train_full[:lookback] = cnn_pred_train_full[lookback]

        cnn_pred_test_full = np.zeros(len(X_test_scaled))
        cnn_pred_test_full[lookback:] = cnn_model.predict(X_test_seq, verbose=0).flatten()
        cnn_pred_test_full[:lookback] = cnn_pred_test_full[lookback]

        cnn_acc = accuracy_score(y_test_seq, (cnn_pred_test_full[lookback:] >= 0.5).astype(int))
        cnn_long_acc = accuracy_score(y_test_seq[y_test_seq==1],
                                      (cnn_pred_test_full[lookback:][y_test_seq==1] >= 0.5).astype(int))
        cnn_short_acc = accuracy_score(y_test_seq[y_test_seq==0],
                                       (cnn_pred_test_full[lookback:][y_test_seq==0] >= 0.5).astype(int))

        models['cnn'] = cnn_model
        base_predictions_train.append(cnn_pred_train_full)
        base_predictions_test.append(cnn_pred_test_full)
        base_accuracies.append(cnn_acc)

        print(f"   ✅ CNN: {cnn_acc*100:.2f}% (Long: {cnn_long_acc*100:.2f}%, Short: {cnn_short_acc*100:.2f}%)")

    else:
        print("\n5/6 - LSTM SKIP (TensorFlow não disponível)")
        print("\n6/6 - CNN SKIP (TensorFlow não disponível)")
    ...
    # Determine optimal threshold for longs
    long_threshold = 0.5
    short_threshold = 0.5
...
if __name__ == "__main__":
    ...
    for days in test_periods:
        ...
        # Train
        model, results = train_ultimate_v5(df, days)
        all_results.append(results)

        # Save
        model_file = storage_dir / f"model_V5_SCALPER_{days}d.pkl"
    ...
    print("\n\n")
    print("=" * 80)
    print("🏆 V5 SCALPER EXTRAORDINÁRIO - RESULTADOS FINAIS")
    print("=" * 80)
    print()
    ...
    print(f"   storage/models/model_V5_SCALPER_{best_days}d.pkl")
    print()
